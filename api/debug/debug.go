...
func (d *Debug) handleTxEnv(blockID thor.Bytes32, txIndex uint64, clauseIndex uint64) (*runtime.Runtime, tx.Transactions, error) {
...
func (d *Debug) traceTransaction(ctx context.Context, tracer vm.Tracer, blockID thor.Bytes32, txIndex uint64, clauseIndex uint64) (interface{}, error) {
	rt, txs, err := d.handleTxEnv(blockID, txIndex, clauseIndex)
	if err != nil {
		return nil, err
	}
	for i, tx := range txs {
...
func storageRangeAt(t *trie.SecureTrie, start []byte, maxResult int) (*StorageRangeResult, error) {
	...
	for i := 0; i < maxResult && it.Next(); i++ {
		...
		e := StorageEntry{Value: thor.BytesToBytes32(content)}
...
func (d *Debug) handleDebugStorage(w http.ResponseWriter, req *http.Request) error {
	...
	res, err := d.debugStorage(req.Context(), opt.ContractAddress, blockID, txIndex, clauseIndex, keyStart, opt.MaxResult)
	if err != nil {
		return err
	}
	return utils.WriteJSON(w, res)
...
func (d *Debug) Mount(root *mux.Router, pathPrefix string) {
	...
	sub.Path("/storage").Methods(http.MethodPost).HandlerFunc(utils.WrapHandlerFunc(d.handleDebugStorage))
